...
const MUTATION_UPDATE_GAME_SESSION_BY_PK = `
  mutation updateGameSessionByPk($closest_guess: Int, $closest_guesser_id: Int, $completion_status: completion_status_enum, $secret_number: Int, $turn_index: Int, $winner_id: Int, $pk_columns: game_session_pk_columns_input!, $game_session_id: Int) {
...
const handler = async (req, res) => {
  let dataTriggerPayload;
  try {
    dataTriggerPayload = await json(req);
  } catch (error) {
    send(res, 400, { error });
    return;
  }

  // MIKE: create a lens for this
  const { game_session_id: gameSessionId, event_type: eventType, user_id: userId, payload } = dataTriggerPayload.event.data.new
